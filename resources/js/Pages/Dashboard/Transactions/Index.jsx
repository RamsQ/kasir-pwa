...
const Index = ({ carts = [], carts_total = 0, products = [], customers = [], discounts = [], paymentSetting = {}, activeShift = null, holds = [] }) => {
        ...
        <>
            ...
            <div id="main-app-content" className={`flex flex-col h-screen w-full transition-colors duration-300 ${isDarkMode ? 'bg-slate-950 text-slate-200' : 'bg-slate-100 text-slate-800'} overflow-hidden print:hidden ${!activeShift ? 'blur-xl pointer-events-none' : ''}`}>
                ...
                <header className="h-16 bg-white dark:bg-slate-900 border-b dark:border-slate-800 flex items-center justify-between px-6 shrink-0 shadow-sm transition-colors">
                    ...
                    <div className="flex items-center gap-3">
                        <button onClick={() => setShowModalHold(true)} className="relative p-2.5 rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 hover:bg-indigo-50 hover:text-indigo-600 transition-all active:scale-90">
                            ...
                            {holds?.length > 0 && (
                                <span className="absolute -top-1 -right-1 h-5 w-5 bg-red-500 text-white text-[9px] font-black flex items-center justify-center rounded-full ring-2 ring-white dark:ring-slate-900 animate-bounce">
                                    {holds.length}
                                </span>
                            )}
                        </button>
                ...
                <main className="flex flex-1 overflow-hidden lg:flex-row flex-col">
                                    ...
                                    <button key={p.id} onClick={() => addToCart(p)} className="text-left group active:scale-95 transition-transform relative">
                                        <div className="bg-white dark:bg-slate-900 p-2 rounded-2xl border border-slate-200 dark:border-slate-800 h-full flex flex-col shadow-sm hover:border-primary-500 transition-all">
                                            <div className="aspect-square bg-slate-100 dark:bg-slate-800 rounded-lg mb-2 overflow-hidden relative">
                                                <img src={p.image ? (p.image.startsWith('http') ? p.image : `/storage/products/${p.image}`) : `https://ui-avatars.com/api/?name=${encodeURIComponent(p.title)}&background=random`} className="w-full h-full object-cover transition-transform group-hover:scale-110" />
                                                {prodDiscount && <div className="absolute top-0 right-0 bg-red-500 text-white text-[9px] font-black px-2 py-1 rounded-bl-xl z-10 animate-pulse flex items-center gap-1 shadow-lg"><IconTag size={10}/> PROMO</div>}
                                            ...
                                            <h3 className="font-bold text-[11px] mb-1 dark:text-white line-clamp-2 h-8 uppercase leading-tight">{p.title}</h3>
                    ...
                    <div className="w-full lg:w-[450px] bg-white dark:bg-slate-900 border-t lg:border-l dark:border-slate-800 flex flex-col h-full shrink-0">
                        <div className="p-4 border-b dark:border-slate-800 flex justify-between items-center font-bold text-sm uppercase shrink-0">
                             ...
                             <span className="bg-primary-50 text-primary-600 px-2.5 py-0.5 rounded-full text-[10px] font-black tracking-tighter uppercase">{carts?.length || 0} ITEM</span>
                        </div>
                                    ...
                                    <div key={c.id} className="p-3 rounded-xl bg-slate-50 dark:bg-slate-800/50 border dark:border-slate-700 flex flex-col shadow-sm">
                                        <div className="flex justify-between items-start mb-1">
                                            <div className="flex-1 min-w-0">
                                                <h4 className="text-[11px] font-bold uppercase truncate dark:text-white flex items-center">
                                                    {c.product?.title}
                                                    {itemDiscount && <span className="ml-2 text-[8px] bg-red-500 text-white px-1.5 py-0.5 rounded-full">PROMO</span>}
                                                </h4>
                                                <p className="font-bold text-xs text-primary-600 dark:text-primary-400">{formatPrice(dPrice)}</p>
                                            </div>
                                            ...
                                            <div className="mb-2 mt-1 pl-2 border-l-2 border-purple-400 text-[9px] text-slate-500 italic bg-purple-50/50 dark:bg-purple-900/10 p-1.5 rounded">
                                                ...
                                                {c.product.bundle_items.map((bi, idx) => (
                                                    <div key={idx} className="flex justify-between">
                                                        <span>• {bi.title}</span>
                                                        <span className="font-bold">x{parseFloat(bi.pivot?.qty || 0) * parseFloat(c.qty)}</span>
                                                    </div>
                                                ))}
                                            </div>
                                        )}

                                        <div className="flex flex-col gap-2 p-2 bg-white dark:bg-slate-900 rounded-xl border border-dashed dark:border-slate-700 mt-2">
                                            <div className="flex items-center justify-between">
                                                <div className="text-[9px] font-bold text-slate-400 uppercase tracking-widest"><IconScale size={14} className="inline mr-1"/> Satuan:</div>
                                                <select value={c.product_unit_id || ''} onChange={(e) => updateCartItem(c.id, c.qty, e.target.value || null)} className="bg-transparent border-none text-[10px] font-black text-primary-600 p-0 focus:ring-0 uppercase cursor-pointer">
                                                    <option value="">Satuan Dasar</option>
                                                    {c.product?.units?.map(u => <option key={u.id} value={u.id}>{u.unit_name} (@{formatPrice(u.sell_price)})</option>)}
                                                </select>
                                            </div>
                                            <div className="flex items-center justify-between border-t dark:border-slate-800 pt-2">
                                                <div className="flex gap-1">
                                                    {[0.25, 0.5].map(v => <button key={v} onClick={() => updateCartItem(c.id, v, c.product_unit_id)} className="text-[8px] bg-slate-100 dark:bg-slate-700 px-2 py-1 rounded font-black hover:bg-primary-500 hover:text-white transition-all">{v}</button>)}
                                                </div>
                        ...
                        <div className="p-4 bg-slate-50 dark:bg-slate-950 border-t dark:border-slate-800 space-y-3 shrink-0">
                            <div className="flex justify-between text-lg font-black text-primary-600 dark:text-primary-400 uppercase tracking-tighter pt-2 border-t border-dashed dark:border-slate-700">
                                <span>TOTAL</span><span>{formatPrice(grandTotal)}</span>
                            </div>

                            <div className="grid grid-cols-4 gap-1.5">
                                {[grandTotal, 10000, 50000, 100000].map((v, i) => (
                                    <button key={i} onClick={() => setCash(v)} className="py-2.5 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-[9px] font-black uppercase shadow-sm hover:bg-primary-50 transition-all dark:text-white">{i === 0 ? 'Pas' : v/1000 + 'K'}</button>
                                ))}
                            </div>
                            ...
                            <div className="grid grid-cols-2 gap-2">
                                <div className="relative flex-1">
                                    ...
                                    <input type="number" value={cash || ''} onChange={(e) => setCash(Number(e.target.value))} placeholder="Tunai..." className="w-full pl-8 pr-3 py-3 text-sm font-black rounded-2xl dark:bg-slate-800 border-slate-200 dark:border-slate-700 dark:text-white shadow-sm focus:ring-primary-500 shadow-inner" />
                                </div>
                                <div className="bg-white dark:bg-slate-800 rounded-2xl border dark:border-slate-700 p-1.5 flex flex-col items-center justify-center shadow-inner text-center">
                                    <span className="text-[8px] font-bold text-slate-400 uppercase leading-none mb-1">Kembali</span>
                                    <span className={`text-xs font-black leading-none ${change >= 0 ? 'text-green-600' : 'text-red-500'}`}>{formatPrice(change)}</span>
                                </div>
                            </div>

                            <div className="flex flex-col gap-2">
                                <button onClick={handleHoldTransaction} disabled={carts.length === 0} className="w-full py-3 border-2 border-dashed border-indigo-200 dark:border-indigo-900 text-indigo-500 hover:bg-indigo-50 dark:hover:bg-indigo-950 font-black text-[10px] rounded-2xl flex items-center justify-center gap-2 uppercase transition-all active:scale-95 shadow-sm">
                                    <IconClockPause size={18} /> Tunda Transaksi
                                </button>
                                <div className="grid grid-cols-2 gap-2">
                                    <button onClick={handlePayCash} disabled={carts.length === 0 || cash < grandTotal} className="py-4 bg-slate-800 text-white font-black text-[10px] rounded-2xl uppercase transition-all shadow-lg active:scale-95 flex flex-col items-center justify-center gap-1 shadow-slate-200 dark:shadow-none"><IconCash size={20} /> Tunai</button>
                                    <button onClick={() => setShowQrisModal(true)} disabled={carts.length === 0} className="py-4 bg-primary-600 text-white font-black text-[10px] rounded-2xl flex flex-col items-center justify-center gap-1 uppercase transition-all shadow-lg active:scale-95 flex items-center justify-center gap-1 shadow-primary-200 dark:shadow-none"><IconQrcode size={20} /> QRIS</button>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            {showQrisModal && (
                <div className="fixed inset-0 z-[150] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
                    <div className="bg-white dark:bg-slate-900 rounded-[2.5rem] p-8 max-w-sm w-full text-center border dark:border-slate-800 shadow-2xl animate-in zoom-in duration-300">
                        <h3 className="text-lg font-black uppercase dark:text-white mb-6">Pembayaran QRIS</h3>
                        <div className="bg-white p-4 rounded-3xl mb-6 border-2 border-slate-100 flex justify-center relative shadow-inner">
                            {paymentSetting?.qris_manual_image ? (
                                <>
                                    <img src={`/storage/payments/${paymentSetting.qris_manual_image}`} className="w-full h-auto max-w-[200px]" />
                                    <button onClick={() => window.print()} className="absolute bottom-2 right-2 p-2.5 bg-primary-600 text-white rounded-xl shadow-lg hover:bg-primary-700 transition-all active:scale-90" title="Cetak Tagihan QRIS"><IconPrinter size={20} /></button>
                                </>
                            ) : <div className="py-14 text-slate-400 italic text-xs uppercase font-bold tracking-widest">QRIS Belum Tersedia</div>}
                        </div>
                        <p className="text-3xl font-black text-primary-600 dark:text-white mb-6 tracking-tighter">{formatPrice(grandTotal)}</p>
                        <div className="flex flex-col gap-3">
                            <button onClick={() => submitTransaction('qris', grandTotal)} className="w-full py-5 bg-primary-600 hover:bg-primary-700 text-white font-black rounded-2xl uppercase text-xs shadow-xl active:scale-95 transition-all">Selesai / Sudah Bayar</button>
            ...
            {showModalHold && (
                <div className="fixed inset-0 z-[150] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
                    <div className="bg-white dark:bg-slate-900 w-full max-w-md rounded-[2.5rem] shadow-2xl overflow-hidden border dark:border-slate-800 animate-in zoom-in duration-300">
                        <div className="p-6 border-b dark:border-slate-800 flex justify-between items-center bg-slate-50/50 dark:bg-slate-800/50">
                            <h3 className="font-black uppercase dark:text-white text-sm flex items-center gap-2"><IconClockPause className="text-indigo-500" /> Daftar Antrean</h3>
                            <button onClick={() => setShowModalHold(false)} className="p-2 bg-slate-100 dark:bg-slate-800 rounded-full text-slate-400"><IconX size={18} /></button>
                        </div>
                        <div className="p-6 max-h-[400px] overflow-y-auto space-y-3 custom-scrollbar">
                            {(!holds || holds.length === 0) ? (
                                <div className="text-center py-10 opacity-30 italic font-bold uppercase tracking-widest text-sm">Belum ada antrean</div>
                            ) : holds.map((h) => (
                                <div key={h.id} className="p-4 bg-slate-50 dark:bg-slate-800/50 rounded-2xl border dark:border-slate-700 flex justify-between items-center hover:border-indigo-300 transition-all group">
                                    <div>
                                        <p className="font-black text-slate-800 dark:text-white uppercase text-xs tracking-tight">{h.ref_number}</p>
                                        <p className="text-[10px] font-bold text-indigo-500 mt-1">{formatPrice(h.total)}</p>
                                    </div>
                                    <div className="flex gap-2">
                                        <button onClick={() => handleResumeHold(h.id)} className="p-2.5 bg-indigo-600 text-white rounded-xl shadow-lg active:scale-90 transition-all" title="Ambil Antrean"><IconRestore size={18}/></button>
                                        <button onClick={() => router.delete(route('holds.destroy', h.id))} className="p-2.5 bg-red-100 text-red-600 rounded-xl hover:bg-red-500 hover:text-white active:scale-90 transition-all" title="Hapus"><IconTrash size={18}/></button>
                                    </div>
                                </div>
                            ))}
                        </div>
                        <div className="p-4 bg-slate-50 dark:bg-slate-800/50 text-center text-[9px] font-bold text-slate-400 uppercase tracking-[0.2em]">Klik biru untuk menarik kembali transaksi</div>
                    </div>
                </div>
            )}
